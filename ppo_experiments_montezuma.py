...
from metric_rnd import ConvRNDModel, args_for_rnd
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.int_critic = layer_init(nn.Linear(512, 1), std=1)

    def get_value(self, x):
        x = self.network(x / 255.0)
        return self.critic(x), self.int_critic(x)
...
if __name__ == "__main__":
    ...
    if args.use_rnd_metric:
        rnd_model = ConvRNDModel(output_size=256).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, int_value = agent.get_action_and_value(
                    next_obs
                )
                values[step] = value.flatten()
                int_values[step] = int_value.flatten()
            actions[step] = action
        ...
        with torch.no_grad():
            ...
            int_advantages = torch.zeros_like(int_rewards).to(device)
            lastgaelam = 0
            lastgaelam_int = 0
            for t in reversed(range(args.num_steps)):
                if t == args.num_steps - 1:
                    ...
                    nextintvalues = next_int_value
                else:
                    nextnonterminal = 1.0 - dones[t + 1]
                    nextvalues = values[t + 1]
                    nextintvalues = int_values[t + 1]
                delta = (
                    rewards[t] + args.gamma * nextvalues * nextnonterminal - values[t]
                )
                delta_int = (
                    int_rewards[t]
                    + args.gamma * nextintvalues * nextnonterminal
                    - int_values[t]
                )
                ...
                int_advantages[t] = lastgaelam_int = (
                    delta_int
                    + args.gamma * args.gae_lambda * nextnonterminal * lastgaelam_int
                )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, newintvalue = (
                ...
                if args.use_rnd_metric:
                    forward_loss = rnd_model.get_forward_loss(b_obs[mb_inds])
                    rnd_model.update(forward_loss)
        ...
        if args.use_model_disagreement_metric:
            writer.add_scalar(
                "losses/disagreement_loss", disagreement_loss.item(), global_step
            )
